...
export class CodepipelineappStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: PipelineStackProps) {
    ...
    // Define the pipeline
    const pipeline = new codepipeline.Pipeline(this, 'MyPipeline5', {
      pipelineName: 'MyS3LambdaPipeline5',
    });
    ...
    pipeline.addStage({
      ...
      actions: [
        new codepipeline_actions.CloudFormationCreateUpdateStackAction({
          ...
          templatePath: sourceOutput.atPath('S3Stack-test.template.json'),
          adminPermissions: true,
        }),
        ...
        new codepipeline_actions.CloudFormationCreateUpdateStackAction({
          ...
          templatePath: sourceOutput.atPath('LambdaStack-test.template.json'),
          adminPermissions: true,
        }),
      ],
    });
